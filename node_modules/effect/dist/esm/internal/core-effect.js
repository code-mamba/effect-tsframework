...
export const gen = function () {
  ...
  return core.suspend(() => {
    ...
    const run = state => state.done ? core.succeed(state.value) : core.flatMap(state.value, val => run(iterator.next(val)));
    return run(state);
  });
};
/* @internal */
...
/** @internal */
export const makeSpan = (name, options) => core.withFiberRuntime(fiber => core.succeed(unsafeMakeSpan(fiber, name, options)));
/* @internal */
